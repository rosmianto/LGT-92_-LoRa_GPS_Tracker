	...
	if (!strcmp(word, "GNRRMC")) {
		...
		if (word != NULL) {
			if (strncmp(lasttime, word, 20)) {
				sscanf(word, "%2d%2d%2d.%4d", &gps.hh, &gps.mm, &gps.ss,
					   &gps.ms);
...
void GPS_INPUT(void) {
	...
	if (loggps == 1) {
		...
		AT_PRINTF("%s:%3d %2d'%5.2f ", (gps.latNS == 'N') ? "North" : "South",
				  dd, mm, ss);
		AT_PRINTF("%s: %.6f\n\r", (gps.latNS == 'N') ? "North" : "South",
				  gps.latitude);

		GPS_DegreeToDMS(gps.longitude, &dd, &mm, &ss);
		AT_PRINTF("%s:%3d %2d'%05.2f", (gps.lgtEW == 'E') ? "East" : "West", dd,
				  mm, ss);
		...
		AT_PRINTF("Time:%2d:%02d:%02d ",
				  (gps.hh < 16) ? gps.hh + 8 : gps.hh - 16, gps.mm, gps.ss);
