...
// static float
// gyro_offsetx=0.005199,gyro_offsety=0.035498,gyro_offsetz=0.005152;
float tmp1, tmp2, tmp3;
float magoffsetx = 1.31454428611172, magoffsety = -1.21753632395713,
      magoffsetz = 1.6567777185719;
float B[6] = {0.980358187761106, -0.0105514731414606, 0.00754899338354401,
              0.950648704823113, -0.0354995317649016, 1.07449478456729};
// float accoffsetx=0.096212,accoffsety=-0.028414,accoffsetz=0.030577;

static float accoffsetx = -0.030587, accoffsety = -0.032310,
             accoffsetz = 0.046341;

float accsensx = 1.00851297697413, accsensy = 0.991366909333871,
      accsensz = 1.00019364448499;
...
uint8_t MPU_Init(void) {
  ...
  MPU_Write_Byte(MPU9250_ADDR, MPU_INTBP_CFG_REG,
                 0X00); // The INT pin is active low,the bypass mode is
                        // enabled,and the magnetometer can be read directly.
  res = MPU_Read_Byte(MPU9250_ADDR, MPU_DEVICE_ID_REG); // Read the ID of
                                                        // MPU9250
  if (res == MPU6500_ID1 || res == MPU6500_ID2) // Correct ID
  {
    MPU_Write_Byte(MPU9250_ADDR, MPU_PWR_MGMT1_REG,
                   0X01); // Set CLKSEL,PLL, X axis as reference
    DelayMs(50);
    MPU_Write_Byte(MPU9250_ADDR, MPU_PWR_MGMT2_REG,
                   0X00); // Acceleration and gyroscope work
    MPU_Set_Rate(200);    // Set sampling frequency 200Hz
  } else
    return 1;
  ...
  if (res == AK8963_ID) {
    ...
    DelayMs(50);
    MPU_Write_Byte(AK8963_ADDR, MAG_CNTL1,
                   0X11); // Set AK8963 as a single measurement
...
void MPU_INT_Init(void) {
  ...
  //  MPU_Write_Byte(MPU9250_ADDR,MPU_PWR_MGMT2_REG,0X00);  	//Acceleration
  //  and gyroscope work
...
uint8_t MPU_Set_Gyro_Fsr(uint8_t fsr) {
  return MPU_Write_Byte(MPU9250_ADDR, MPU_GYRO_CFG_REG,
                        (fsr << 3) | 3); // ���������������̷�Χ
}
...
uint8_t MPU_Set_Accel_Fsr(uint8_t fsr) {
  return MPU_Write_Byte(MPU9250_ADDR, MPU_ACCEL_CFG_REG,
                        fsr << 3); // ���ü��ٶȴ����������̷�Χ
}
...
uint8_t MPU_Set_LPF(uint16_t lpf) {
  ...
  if (lpf >= 188)
    ...
    data = 6;
  return MPU_Write_Byte(MPU9250_ADDR, MPU_CFG_REG,
                        data); // Set the digital low-pass filter
}
...
uint8_t MPU_Set_Rate(uint16_t rate) {
  ...
  if (rate < 4)
    rate = 4;
  data = 1000 / rate - 1;
  data = MPU_Write_Byte(MPU9250_ADDR, MPU_SAMPLE_RATE_REG,
                        data); // Set the digital low-pass filter
  return MPU_Set_LPF(
      rate / 2); // Automatically set LPF to half of the sampling frequency
}
...
void calibrate1(void) {
  ...
  float ax2, ay2, az2;
  float gx2, gy2, gz2, sumx = 0, sumy = 0, sumz = 0, sumx1 = 0, sumy1 = 0,
                       sumz1 = 0;
...
/////////////////////////////////
// Get the gyro value(original value),filter the average of the source data,and
// adjust the gyroscope orientation gx,gy,gz:Raw data of the gyroscope X, Y, Z
// axis (with symbol) Return value: 0,set successfully
//           Other,error code
...
// Get the acceleration value(original value),low pass filter and adjust
// acceleration orientation ax,ay,az:Raw data of the acceleration X, Y, Z axis
// (with symbol) Return value: 0,set successfully
//           Other,error code
...
// mx,my,mz:������x,y,z���ԭʼ����(������)
// Get the acceleration value(original value),low pass filter and adjust
// acceleration orientation mx,my,mz:Raw data of the acceleration X, Y, Z axis
// (with symbol) Return value: 0,set successfully
//           Other,error code
